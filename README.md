...
[![npm version](https://img.shields.io/npm/v/env-validator-gen)](https://www.npmjs.com/package/env-validator-gen)
[![npm downloads](https://img.shields.io/npm/dm/env-validator-gen)](https://www.npmjs.com/package/env-validator-gen)
...
**[View on npm →](https://www.npmjs.com/package/env-validator-gen)**

## Install

```bash
# As a dev dependency (recommended)
npm install -D env-validator-gen
...
Scans every `.ts`, `.tsx`, `.js`, `.jsx` file under `./src`, finds all `process.env.*` usages, and writes `src/env.ts` — a Zod schema that validates everything at startup.
...
```ts
// src/main.ts
import './env'; // validates at startup — crashes with a clear message if anything is missing
```
...
With `--raw`, generates a plain validator with zero dependencies:
...
| | |
|---|---|
| **Scanned extensions** | `.ts`, `.tsx`, `.js`, `.jsx`, `.mjs`, `.cjs` |
| **Ignored directories** | `node_modules`, `dist`, `.git`, `coverage`, `.next`, `build` |

Variables are deduplicated and sorted alphabetically in the output.
...
// Or scan a string of code directly
